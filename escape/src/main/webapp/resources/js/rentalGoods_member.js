...
/**********************************다른 업체 상품 출력 ******************************************** */
function getOtherGoods(pPageNumber, pAmount, pCfCode, pCaCode){
	let jsonData = [];
		jsonData.push({
		...
		getAjaxJson("GetOtherStoresGoods", clientData, "getOtherStoreGoods");
}
/************************************상품 전체 리스트 ************************************************/
/*모든 업체 상품 불러오기*/
...
function getList(jsonData){
	...
	const gdList = createDiv("gdList","gdList");
		...
		for(let x=0; x<jsonData.length; x++){
			...
			bigDiv.innerHTML += "<button id='btnCart'class='twoBtn' onClick=\"openCart('"+jsonData[x].goCode+"','"+jsonData[x].srCode+"','"+jsonData[x].cfCode+"','"+jsonData[x].caCode+"','1')\">장바구니</button>" + "<button  id='btnBuy'class='twoBtn' onClick=\"openBuy('"+jsonData[x].goCode+"','"+jsonData[x].srCode+"','"+jsonData[x].cfCode+"','"+jsonData[x].caCode+"','2')\">바로구매</button>";
...
function openCart(pGoCode,pSrCode,pCfCode,pCaCode,pBtnType){
	...
	const clientData = JSON.stringify(jsonData);
	getAjaxJson("GetThatStoreThatGoods", clientData, "getModalInfo");
	
}	
//사용 월, 사용 일 , 대여기간 선택 시 div:goCart 채우기
function changeModal(){
	const month = document.getElementById("modalMonth");
	const day = document.getElementById("modalDay");
	const rentPrice = document.getElementById("modalPrice");
	...
	if(month.options[month.selectedIndex].value != ''){
		...
		if(day.options[day.selectedIndex].value != ''){
			...
			if(rentPrice.options[rentPrice.selectedIndex].value != ''){
				...
				const cart = document.getElementById("goCart1");
				...
				//글씨
				data += "<div name='goOrder' class='goOrder1'>";
				data +="<div name='goName' class='goName'>"+goName+"</div>";
				data += "<span id='selectMonth"+i+"'>"+selectMonth+"월/"+"</span>";
				data += "<span id='selectDay"+i+"'>"+selectDay+"일/"+"</span>";
				...
				data += "</div>";
				data += "<div name='goOrder3' class='goOrder13'>";
				data += "<button name='inc' class='inc1' onClick='modQty1(\"plus\","+i+")'>△</button>";
				data += "<button name='dec' class='dec1' onClick='modQty1(\"minus\","+i+")'>▽</button>";
				data += "</div>";
				data += "<div name='goOrder4' class='goOrder4'>";
				data += "<button name='delOrder' class='delOrder1' onClick='modQty1(\"delete\","+i+")'>X</button>";
				data += "</div>";
				data += "</div>";
				...
				document.getElementById("goCart1").innerHTML += data;
				document.getElementById("goCart1").style.borderBottom = '2px solid #8C8C8C';
...
//상품 전체 리스트 : 장바구니 클릭 : modal open
function openBuy(pGoCode,pSrCode,pCfCode,pCaCode,pBtnType){
	let jsonData = [];
	jsonData.push({
				goCode : pGoCode,
				srCode : pSrCode,
...
//Modal띄우기 function :: 장바구니, 바로구매 버튼 분리해야함
let goModal1 = ["image","srName","goName","stCode","discount","month","day","rentPrice","srCode","goCode","cfCode","caCode"];

function getModalInfo(jsonData){
	delCartModal();
	
	 var offset = $("#openCart").offset();
      $('html, body').animate({scrollTop : offset.top}, 400);

	const div = document.getElementById("openCart");
	div.scrollIntoView();
	div.style.display = "block";
	const goModal = createDiv("goInfo","goInfo");
	if(jsonData.stCode == "S002"){
		//월 나누기
	if(jsonData.month.indexOf(",")>0){
		let month =  jsonData.month.split(",");
			jsonData.month = "";
			jsonData.month = "<div id='cal3'>사용시작 월"
			jsonData.month += "<select id='modalMonth' class='sel' onchange='resetModal()' disabled>" ;
	...
	if(jsonData.day.indexOf(",")>0){
		let day =  jsonData.day.split(",");
			jsonData.day = "";
			jsonData.day = "<div id='cal4'>사용시작 일";
			jsonData.day += "<select id='modalDay' class='sel' disabled>" ;
	...
	if(jsonData.rentPrice.indexOf("&")>0){
		...
		jsonData.rentPrice += "<div id='cal5'>대여기간&가격";
		jsonData.rentPrice += "<select id='modalPrice' class='sel' onchange='changeModal()' disabled>" ;
		...
		jsonData.rentPrice += "</select></div><div id='goCart1' ></div><div id='total1'><span>총 상품 금액(수량) : </span><span id='totPrice' class='pr'>0</span><span class='pr'>원</span><span class='qty'>(</span><span id='totQty' class='qty'>0</span><span class='qty'>개</span><span class='qty'>)</span></div>";
	    if(jsonData.btnType == "1"){
         jsonData.rentPrice   += "<div class='btnDiv'><button class='modalBtn' >장바구니</button><button  class='modalClose' onClick='closeCartModal()'>닫기</button></div>";
      }else if(jsonData.btnType == "2"){
         jsonData.rentPrice   += "<div class='btnDiv'><button id='btn2' class='modalBtn' onClick=''>결제</button><button class='modalClose' onClick='closeCartModal()'>닫기</button></div>";
      }
	...
	if(jsonData.month.indexOf(",")>0){
		let month =  jsonData.month.split(",");
			jsonData.month = "";
			jsonData.month = "<div id='cal3'>사용시작 월"
			jsonData.month += "<select id='modalMonth' class='sel' onchange='resetModal()'>" ;
			jsonData.month += "<option value = ''selected='selected'/>--  [필수] 사용월 선택 --</option>";
		for(let x=0; x<12; x++){
			jsonData.month += "<option value='"+month[x]+"'>"+month[x]+"월</option>";
		}
	...
	if(jsonData.day.indexOf(",")>0){
		let day =  jsonData.day.split(",");
			jsonData.day = "";
			jsonData.day = "<div id='cal4'>사용시작 일";
			jsonData.day += "<select id='modalDay' class='sel' >" ;
			jsonData.day += "<option value = '' selected='selected' />--  [필수] 사용일 선택  --</option>";
		for(let x=0; x<31; x++){
			jsonData.day += "<option value='"+day[x]+"'>"+day[x]+"일</option>";
		}
	...
	if(jsonData.rentPrice.indexOf("&")>0){
		...
		jsonData.rentPrice += "<div id='cal5'>대여기간&가격";
		jsonData.rentPrice += "<select id='modalPrice' class='sel' onchange='changeModal()'>" ;
		jsonData.rentPrice += "<option value = '' selected='selected'/>--  [필수] 대여기간 선택  --</option>";
		for(let y=0; y<rent.length;  y++){
		...
		jsonData.rentPrice += "</select></div><div id='goCart1' ></div><div id='total1'><span>총 상품 금액(수량) : </span><span id='totPrice' class='pr'>0</span><span class='pr'>원</span><span class='qty'>(</span><span id='totQty' class='qty'>0</span><span class='qty'>개</span><span class='qty'>)</span></div>";
	    if(jsonData.btnType == "1"){
         jsonData.rentPrice   += "<div class='btnDiv'><button class='modalBtn' onClick=\"saveModal('"+jsonData.cfCode+"','"+jsonData.caCode+"','"+jsonData.srCode+"','"+jsonData.goCode+"')\">장바구니</button><button  class='modalClose' onClick='closeCartModal()'>닫기</button></div>";
      }else if(jsonData.btnType == "2"){
         jsonData.rentPrice   += "<div class='btnDiv'><button id='btn2' class='modalBtn' onClick=''>결제</button><button class='modalClose' onClick='closeCartModal()'>닫기</button></div>";
      }
	...
	for(let l=0; l<1; l++){
		for(let i=0; i<12; i++){
		let column = createDiv(goModal1[i],"goModal "+goModal1[i]);
		column.innerHTML = 
							(i==0)? ((jsonData.cfCode == "1000")? "<img id='modalImage' src='resources/images/cp/"+jsonData.image+"' />" : "<img id='modalImage' src='resources/images/fi/"+jsonData.image+"' />") :
								(i==1)? "[" + jsonData.srName + "]" :
									(i==2)? jsonData.goName :
										(i==3)? ((jsonData.stCode == "S001")? "<span id='grey'>렌탈 가능</span>" : "<span id='red'>렌탈 불가</span>") :
															...
															(i==8)? jsonData.srCode :
																(i==9)? jsonData.goCode :
																	(i==10)? jsonData.srCode : 
																		(i==11)? jsonData.cfCode : 
																			jsonData.cfCode;
																			 
		goModal.appendChild(column);						
		}
	}
	div.appendChild(goModal);
	
	
	
}
//장바구니에 insert
function saveModal(pCfCode,pCaCode,pSrCode,pGoCode){
	
	let jsonData = [];
	
	
	const item = document.getElementById("goCart1");
	 
	
	for(i=0; i<item.children.length; i++){
		const mmId= document.getElementById("mmId").innerHTML;
		const pMmId = mmId.substring(0,mmId.indexOf("님"));
		
		const order = document.getElementById("orderDiv"+i);
		
		//월
		const pmonth = document.getElementById("selectMonth"+i).innerHTML;
		const selectMonth = pmonth.substring(0,pmonth.indexOf("월"));
		
		
		//일
		const pday = document.getElementById("selectDay"+i);
		const selectDay = pday.innerHTML.substring(0,pday.innerHTML.indexOf("일"));
		
		
		//몇박몇일/가격
		const rent = order.querySelector(".nodePrice");
		let selectRent = "";
		if(rent.innerText.indexOf("(")>0){
			selectRent = rent.innerText.substring(0,rent.innerText.indexOf("("));
		}else {
			selectRent = rent.innerText;
		}
		
		
		//수량
		const pQty = document.getElementById("modQty"+i).innerText;
	...
	jsonData.push({
			...
			goCode:pGoCode,
			qty:pQty,
			rentDay:selectMonth+"/"+selectDay,
			rentPrice:selectRent
	});
	}
	
	
	let clientData = JSON.stringify(jsonData);
	getAjaxJson("InsCart",clientData,"getCartPage")
	
}
//장바구니 저장 후 다시 오기
function getCartPage(jsonData){
	closeCartModal();
	...
	let data = "";
		...
		data += "<div id='question'>장바구니에 선택하신 상품이 담겼습니다.</div>";
		data += "<div id='goLogIn'onClick=\"getNewPageMember('"+jsonData.mmId+"','cart_member','"+jsonData.publicIp+"','"+jsonData.mmCode+"')\">장바구니</div>";
...
function closeCartModal(){
	const cartModal = document.getElementById("openCart");
	cartModal.style.display = "none";
}

//장바구니,바로구매 modal 내용 삭제하기
function delCartModal(){
	const cartModal = document.getElementById("openCart");
...
//select 박스 modal 초기화
function resetModal(){
$("#modalDay option:eq(0)").prop("selected", true);
$("#modalPrice option:eq(0)").prop("selected", true);	
}

//수량 증가 시키기
function modQty1(name, i){
	const order = document.getElementById("orderDiv"+i);
...
function getGoodsInfo(jsonData){
	...
	if(jsonData.rentPrice.indexOf("&")>0){
		...
		jsonData.rentPrice += "</select></div><div id='goCart' ></div><div id='total'><span>총 상품 금액(수량) : </span><span id='totPrice' class='pr'>0</span><span class='pr'>원</span><span class='qty'>(</span><span id='totQty' class='qty'>0</span><span class='qty'>개</span><span class='qty'>)</span></div>";
	    jsonData.rentPrice	+= "<button id='btn1'>바로구매</button><button id='btn2' onClick=\"saveInfo('"+jsonData.cfCode+"','"+jsonData.caCode+"','"+jsonData.srCode+"','"+jsonData.goCode+"')\">장바구니</button></div>";
		console.log(jsonData.rentPrice);
	}
	}
...
//장바구니에 insert
function saveInfo(pCfCode,pCaCode,pSrCode,pGoCode){
	
	let jsonData = [];
	
	
	const item = document.getElementById("goCart");
