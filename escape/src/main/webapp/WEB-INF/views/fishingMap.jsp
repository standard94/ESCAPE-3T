   ...
   <div id="basic">
      ...
      <select id="job" name="job" size="1">
         ...
         <option value="조아요순">찜순</option>
               ...
               <div id="listPoints">
                  <div id="mySideStore" onClick="delrecord(); getNewPage('caravanMap')">카라반</div>
                  <div id="fishingPoint" onClick="delrecord(); getNewPage('glampingMap')">글램핑</div>
                  <div id="camping" onClick="delrecord(); getNewPage('campingMap');">캠핑</div>
                  <div id="glamping" onClick="getNewPage('fishingMap');">낚시 포인트</div>
...
<script>
	...
	function myMarker(locPosition, message, pHaName, caCode) {
		var haName;
		delLeftList();	//좌측 낚시포인트 리스트 지우기
		delOpenOverlay();	//열려있는 낚시포인트 오버레이 지우기
		setSrMarkers(null);	//렌탈 업체 지도에서 지우기
		...
		//내 지역에 표시된 낚시포인트 리스트 요청
		getFishingList(caCode, '2000', haName);
		
		//내 지역에 표시된 낚시포인트 좌측 리스트 페이지 수량 계산
		getFishingPage('1','10', caCode, '2000', haName);
		
		//내 지역에 표시된 낚시포인트 좌측 리스트 요청
		chooseFishingList('1','10', caCode,'2000', haName);
	...
	//낚시포인트 리스트 가져오기
	function getFishingList(pCaCode, pCfCode, pHaName) {
		...
		//서버 요청
		getAjaxJson("GetFishingList", clientData, "setPoint");
		}
				...
				geocoder.addressSearch(list[n].maAddress, function(result, status) {
					...
					if (status === kakao.maps.services.Status.OK) {
						...
						addMarker(new kakao.maps.LatLng(result[0].y, result[0].x),
								list[n].fpCaCode, list[n].fpCaName, list[n].fpCode, list[n].fpContents, list[n].fpDate, list[n].fpFiCode, list[n].FiName, list[n].fpGeName, list[n].fpImage, list[n].fpMaCfCode, list[n].fpMaMmCode, list[n].fpMaPost, list[n].fpThCode, list[n].fpThName, list[n].fpTitle, list[n].fpView, list[n].maAddress, list[n].mmId,
	...
	function addMarker(position, fpCaCode, fpCaName, fpCode, fpContents, fpDate, fpFiCode, fpFiName,
			fpGeName, fpImage, fpMaCfCode, fpMaMmCode, fpMaPost, fpThCode, fpThName, fpTitle, fpView, maAddress, mmId, wishPos) {
		...
		kakao.maps.event.addListener(marker, 'click', function() {
			...
			findFishingDetail(fpCaCode, fpMaCfCode, fpCode);
			overlay.setMap(map);
		...
		makeOverlay(maAddress, fpImage, fpFiName, mmId, overlay);
		}
	...
	function addSrMarker(position, name, address, phone, img, srCf, sr, wishPos) {
	...
	function chooseFishingList(pPageNumber, pAmount, pFpCaCode, pFpMaCfCode, pHaName) {
		let jsonData = [];
		...
		jsonData.push({
			...
			amount : pAmount,
			fpCaCode : pFpCaCode,
			fpMaCfCode : pFpMaCfCode,
			haName : pHaName
			});
		
		const clientData = JSON.stringify(jsonData);
		
		getAjaxJson("ChooseFishingList", clientData, "getPoint");
		}
		
	//캠핑 페이지 번호 불러오기
	function getFishingPage(pPageNumber, pAmount, pFpCaCode, pFpMaCfCode, pHaName) {
		let jsonData = [];
		...
		getAjaxJson("GetFishingPage", clientData, "alertPage");
		}
			...
			new daum.Postcode({
				oncomplete : function(data) {
					...
					geocoder.addressSearch(data.address, function(result, status) {
						...
						if (status === kakao.maps.services.Status.OK) {
							// 마커 하나를 지도위에 표시합니다
							myMarker(new kakao.maps.LatLng(result[0].y, result[0].x),
	...
	function alertPage(jsonData) {
		let page = document.getElementById("fishingPage");
		let pageInfo = "";
		let startPage = jsonData.startPage - 1;
		let endPage = jsonData.endPage + 1;
		makePage(jsonData.haName, page, pageInfo, startPage, endPage, jsonData.fpCaCode, jsonData.fpMaCfCode, jsonData.pageNumber, jsonData.next, jsonData.prev);
		}
	
	function makePage(address, page, pageInfo, startPage, endPage, jsonFpCaCode, jsonFpMaCfCode, jsonPageNumber, jsonNext, jsonPrev){
		var haName = address.substring(0,2);
		if (jsonPrev != false) {
			pageInfo = "<div class='pageInfo_btn_prev'><li onClick =\"delrecord();getFishingPage('"
					+ startPage
					+ "','10','"
					+ jsonFpCaCode
					+ "','"
		...
		for (i = startPage+1; i < endPage; i++) {
			if (i == jsonPageNumber) {
				pageInfo += "<div class='pageInfo_btn_selected' style='width:4%;' ><li onClick =\"delrecord();getFishingPage('"
						+ i
						+ "','10','"
						+ jsonFpCaCode
						+ "','"
						...
						+ "</li></div>";
							...
							pageInfo += "<div class='pageInfo_btn' style='width:4%;'><li onClick =\"delrecord();getFishingPage('"
									+ i
									+ "','10','"
									+ jsonFpCaCode
									+ "','"
		...
		if (jsonNext != false) {
			pageInfo += "<div class='pageInfo_btn_next'><li onClick =\"delrecord();getFishingPage('"
					+ i
					+ "','10','"
					+ jsonFpCaCode
					+ "','"
	...
	const columnName = [ "fpImage", "fpName", "maAddress", "haName", "thName", "fpNumber" ];
		...
		if(list[0].maAddress!=null){
			for (let x = 0; x < list.length; x++) {
				geocoder.addressSearch(list[x].maAddress, function(result, status) {
					...
					if (status === kakao.maps.services.Status.OK) {
						// 마커 하나를 지도위에 표시합니다
						getLeftFishingList(new kakao.maps.LatLng(result[0].y, result[0].x),
								list[x].fpCaCode, list[x].fpCaName, list[x].fpCode, list[x].fpContents, list[x].fpDate,
								list[x].fpFiCode, list[x].fpFiName, list[x].fpGeName, list[x].fpImage, list[x].fpMaCfCode,
								list[x].fpMaMmCode, list[x].fpMaPost, list[x].fpThCode, list[x].fpThName, list[x].fpTitle,
								list[x].fpView, list[x].maAddress, list[x].mmId,
								new kakao.maps.LatLng( parseFloat(result[0].y) + 0.11, parseFloat(result[0].x) + 0.23));
						}
					});
				}
	...
	function getLeftFishingList(mPos, fpCaCode, fpCaName, fpCode, fpContents, fpDate,
			fpFiCode, fpFiName, fpGeName, fpImage, fpMaCfCode, fpMaMmCode, fpMaPost,
			fpThCode, fpThName, fpTitle, fpView, maAddress, mmId, wishPos) {
		...
		for (let i = 0; i < 6; i++) {
			...
			column.innerHTML = (i == 0) ? "<img id='fImage' src=resources/images/camping/"+fpImage+"/>"
							: (i == 1) ? maAddress : (i == 2) ? "ID: " + mmId : (i == 3) ? fpThName : (i == 4) ? "#" + fpFiName : fpDate;
		...
		makeOverlay(maAddress, fpImage, mmId, fpFiName, overlay2);
		var haName;
		fpList.onclick = function() {
			
			if(maAddress.substring(0,1)=="충" || maAddress.substring(0,1)=="전"){
		   		haName = maAddress.substring(0,1) + maAddress.substring(2,3);
		   	}else if(maAddress.substring(0,1)=="경"){
		   		if(maAddress.substring(1,2)=="기"){
		   			haName = maAddress.substring(0,2);
		   		}else{
		   			haName = maAddress.substring(0,1) + maAddress.substring(2,3);
		   		}
		   	}else{
			...
			getFishingList(fpCaCode, '2000', haName);
			findFishingDetail(fpCaCode, fpMaCfCode, fpCode);
	...
	//특정 캠핑장 상세정보값 보내기
	function findFishingDetail(pFpCaCode, pFpMaCfCode, pFpCode) {
		let jsonData = [];
		...
		getAjaxJson("findFishingDetail", clientData, "getFishingDetail");
		}
   ...
   function getFishingDetail(detail) {
	   console.log(detail)
         ...
         title.id = "TitleBox"
         title.innerHTML = detail[0].fpTitle;
   ...
   function nearStore(){
	   ...
	   getStoreList('2000');
	   getStorePage('1','10', '2000', haName);
	   chooseStoreList('1','10', '2000', haName);
   }
   ...
   function getLeftStoreList(mPos, srI, srN, srA, haN, srNb, srCf, srCd, wishPos) {
   ...
   function delLeftList(){
	   var fsList = document.getElementById("fishingList");
	   var page = document.getElementById("fishingPage");
	   ...
	   while (fsList.hasChildNodes()) {
	   fsList.removeChild(fsList.firstChild);
	   }
   }
   
   function delOpenOverlay(){
		  if (clickedoverlay) {
		         clickedoverlay.setMap(null);
		      }
	  }
